...
class SSLSegmenter(LightningModule):
    ...
    def shared_step(self, batch, batch_idx, split):
        logit = self.model(batch['imgs'])  # 一张与原图一样大的单通道图片
        logit = logit.squeeze(dim=1)  # 由于单通道，因此压掉通道维度
        loss = self.loss(logit, batch['masks'])  # 计算损失
        prob = torch.sigmoid(logit)  # 转化为概率
        dice = self.get_dice(prob, batch['masks'])  # 计算dice值
        '''
        if batch_idx == 0:  # 无用
        ...
        #         path = BUSI_SEG_VISUAL_PRED / ('pred_' + filename)
