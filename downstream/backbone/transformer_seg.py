...
from downstream.backbone.transformer_model import TransModel2d, TransConfig
import math
...
class Encoder2D(nn.Module):
    def __init__(self, config: TransConfig, is_segmentation=True):
        ...
        self.bert_model = TransModel2d(config)  # vit encoder
...
class SETRModel(nn.Module):
    def __init__(self, patch_size=(32, 32),
        ...
        self.decoder_2d = Decoder2D(in_channels=config.hidden_size,
                                    out_channels=config.out_channels, features=decode_features)  # 一系列卷积上采样，恢复原图大小，通道数降为1
