...
class MIMICImageDataset(BaseImageDataset):
    def __init__(self, split="train", transform=None, data_pct=1.0, img_type="Frontal", imsize=256):
        ...
        # replace uncertains  # 将疾病类别标签为不确定的置为1或者0，使得所有的标签都为01
        uncertain_mask = {k: -1 for k in CHEXPERT_COMPETITION_TASKS}
        self.df = self.df.replace(uncertain_mask, CHEXPERT_UNCERTAIN_MAPPINGS)
    ...
    def __getitem__(self, index):
        ...
        # get labels
        y = list(row[CHEXPERT_COMPETITION_TASKS])  # 各个疾病的01标签
        y = torch.tensor(y)
...
class COVIDXImageDataset(BaseImageDataset):
    def __init__(self, split="train", transform=None,
        ...
        if data_pct != 1 and self.split == "train":
            self.df = self.df.sample(frac=data_pct, random_state=42)  # frac：采样比例，即从中抽取data_pct的数据作为训练数据
...
class BUSIImageDataset(BaseImageDataset):
    def __init__(self, split="train", transform=None,
        ...
        if self.split == "train":
            self.df = pd.read_csv(BUSI_CLS_TRAIN)
            self.df["filename"] = self.df["filename"].apply(
                lambda x: BUSI_DATA + '/' + f"files/{x}")
        elif self.split == "valid":
            self.df = pd.read_csv(BUSI_CLS_VAL)
            self.df["filename"] = self.df["filename"].apply(
                lambda x: BUSI_DATA + '/' + f"files/{x}")
        elif self.split == "test":
            self.df = pd.read_csv(BUSI_CLS_TEST)
            self.df["filename"] = self.df["filename"].apply(
                lambda x: BUSI_DATA + '/' + f"files/{x}")
        else:
            raise ValueError(f"split {split} does not exist!")
...
if __name__ == '__main__':
    ...
    dataset = AUIDTImageDataset(split='train', transform=transform)
    print(dataset[0])
