...
def get_batch(data_iterator, args, timers):
    ...
    data_i0 = mpu.broadcast_data(['img0'], data, torch.float32)
    data_i1 = mpu.broadcast_data(['img1'], data, torch.float32)
    # Unpack.
    ...
    return tokens, labels, img0, img1, data['pre_image']
...
def forward_step(data_iterator, model, args, timers):
    ...
    tokens, labels, img0, img1, pre_image = get_batch(
        data_iterator, args, timers)
    timers('batch generator').stop()

    # logits = model(input_ids=tokens, image=image, pre_image=pre_image)[0]
    logits = model(input_ids=tokens, img0=img0, img1=img1, pre_image=pre_image)[0]
    dtype = logits.dtype
...
class UltrasoundDataset(Dataset):
    def __init__(self, path, processor, tokenizer, args):
        ...
        self.labels = []
        for item in data:
            img_list = self.genearte_img_list(item['img'])
            if len(img_list) != 2:
                continue
            input0 = tokenizer.encode("<img>", add_special_tokens=False)
            input1 = [tokenizer.pad_token_id] * args.image_length
    ...
    def genearte_img_list(self, path):
        img_list = []
        for i in range(1, 50):
            p = path.replace('k', str(i))
            if os.path.isfile(p):
                img_list.append(p)
            else:
                break
    ...
    def __getitem__(self, idx):
        # img_path = random.sample(self.images[idx], 1)[0]
        # img = self.processor(Image.open(img_path).convert('RGB'))
        img0_path = self.images[idx][0]
        img1_path = self.images[idx][-1]
        img0 = self.processor(Image.open(img0_path).convert('RGB'))
        img1 = self.processor(Image.open(img1_path).convert('RGB'))
        return {
            # "image": img,
            "img0": img0,
            "img1": img1,
...
def create_ultrasound_dataset_function(path, args):
    tokenizer = get_tokenizer(args)
    image_processor = BlipImageEvalProcessor(224)

    dataset = UltrasoundDataset(path, image_processor, tokenizer, args)
    return dataset
...
if __name__ == '__main__':
    ...
    def data_collator(examples):
        ...
        ret = {
            ...
            'img0': torch.stack([example['img0'] for example in examples]),
            'img1': torch.stack([example['img1'] for example in examples]),
            'pre_image': example['pre_image']
        }
        return ret
    training_main(args, model_cls=model, forward_step_function=forward_step, create_dataset_function=create_ultrasound_dataset_function, collate_fn=data_collator)