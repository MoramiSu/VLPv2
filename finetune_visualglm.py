...
class FineTuneVisualGLMModel(VisualGLMModel):
    def __init__(self, args, transformer=None, parallel_output=True, **kw_args):
        ...
        if args.use_lora:
            self.add_mixin("lora", LoraMixin(args.num_layers, args.lora_rank, layer_range=args.layer_range),
                           reinit=True)
...
def get_batch(data_iterator, args, timers):
    ...
    # data_i0 = mpu.broadcast_data(['image0'], data, torch.float32)
    # data_i1 = mpu.broadcast_data(['image1'], data, torch.float32)
    # Unpack.
...
def forward_step(data_iterator, model, args, timers):
    ...
    # tokens, labels, image0, image1, pre_image = get_batch(
    #     data_iterator, args, timers)
    tokens, labels, image, pre_image = get_batch(
        data_iterator, args, timers)
    timers('batch generator').stop()

    # logits = model(input_ids=tokens, image0=image0, image1=image1, pre_image=pre_image)[0]\
...
class UltrasoundDataset(Dataset):
    ...
    #     img1 = self.processor(Image.open(img0_path).convert('RGB'))
...
if __name__ == '__main__':
    ...
    def data_collator(examples):
        ...
        ret = {
            ...
            # 'image0': torch.stack([example['image0'] for example in examples]),
            # 'image1': torch.stack([example['image1'] for example in examples]),
