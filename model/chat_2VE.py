...
def process_image(text, image=None):
    ...
    # return text, image_position, image
            img0 = Image.open(image_path.replace('k', '1'))
            img1 = Image.open(image_path.replace('k', '2'))
    processor = BlipImageEvalProcessor()
    img0 = processor(img0.convert('RGB'))
    img0 = img0.unsqueeze(0)
    img1 = processor(img1.convert('RGB'))
    img1 = img1.unsqueeze(0)
    return text, image_position, img0, img1
...
def chat(image_path, model, tokenizer,
    ...
    prompt, image_position, img0, img1 = process_image(prompt, image=image)
    img0 = img0.to(next(model.parameters()).dtype).to(next(model.parameters()).device)
    img1 = img1.to(next(model.parameters()).dtype).to(next(model.parameters()).device)
