...
class ImageMixin(BaseMixin):
    ...
    def word_embedding_forward(self, input_ids, output_cross_layer, **kw_args):
        # if kw_args["pre_image"] > input_ids.shape[1] or kw_args.get("image0", None) or kw_args.get("image1", None) is None:
        #     return self.transformer.word_embeddings(input_ids)
        # image0_emb = self.model(image=kw_args.get("image0"), **kw_args)
        # image1_emb = self.model(image=kw_args.get("image1"), **kw_args)
        ...
        # return torch.cat([pre_txt_emb, image0_emb, image1_emb, post_txt_emb], dim=1)
...
class VisualGLMModel(ChatGLMModel):
    ...
    def add_model_specific_args(cls, parser):
        ...
        group.add_argument('--image_length', type=int, default=64)
