...
class UniVLPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    @classmethod  # 类方法，可在实例化前调用，其中cls=self，代表UniVLPreTrainedModel
    def from_pretrained(cls, pretrained_bert_name, visual_model_name, cross_model_name, decoder_model_name,
        ...
        bert_config, state_dict = BertConfig.get_config(pretrained_bert_name, cache_dir, type_vocab_size, state_dict, task_config=task_config)  # BERT配置
        visual_config, _ = VisualConfig.get_config(visual_model_name, cache_dir, type_vocab_size, state_dict=None, task_config=task_config)  # 视觉编码器配置
        cross_config, _ = CrossConfig.get_config(cross_model_name, cache_dir, type_vocab_size, state_dict=None, task_config=task_config)  # 跨模态编码器配置
        decoder_config, _ = DecoderConfig.get_config(decoder_model_name, cache_dir, type_vocab_size, state_dict=None, task_config=task_config)  # 解码器配置

        model = cls(bert_config, visual_config, cross_config, decoder_config, *inputs, **kwargs)  # UniVL初始化

        assert model.bert is not None
        assert model.visual is not None

        if state_dict is not None:
            model = cls.init_preweight(model, state_dict, task_config=task_config)  # 加载预训练权重
...
class NormalizeVideo(nn.Module):
    ...
    def forward(self, video):
        video = torch.as_tensor(video).float()  # 拷贝
        video = video.view(-1, video.shape[-2], video.shape[-1])  # 压掉第二个维度
        video = self.visual_norm2d(video)  # 特征归一化
        return video
...
class UniVL(UniVLPreTrainedModel):
    def __init__(self, bert_config, visual_config, cross_config, decoder_config, task_config):
        ...
        if check_attr('stage_two', self.task_config):  # 任务配置中若有stage_two的配置且为真
        ...
        bert_config = update_attr("bert_config", bert_config, "num_hidden_layers",
                                   self.task_config, "text_num_hidden_layers")  # 用任务配置中的BERT层数替换BERT配置中的层数
        self.bert = BertModel(bert_config)  # BERT初始化
        bert_word_embeddings_weight = self.bert.embeddings.word_embeddings.weight  # BERT的词嵌入权重
        bert_position_embeddings_weight = self.bert.embeddings.position_embeddings.weight  # BERT的位置编码权重
        # <=== End of Text Encoder

        # Video Encoder ===>
        visual_config = update_attr("visual_config", visual_config, "num_hidden_layers",
                                    self.task_config, "visual_num_hidden_layers")  # 用任务配置中的视觉编码器层数替换视觉编码器配置中的层数
        self.visual = VisualModel(visual_config)  # 视觉编码器初始化
        visual_word_embeddings_weight = self.visual.embeddings.word_embeddings.weight  # 视觉嵌入权重
        ...
        if self._stage_one is False or self.train_sim_after_cross:
            ...
            cross_config = update_attr("cross_config", cross_config, "num_hidden_layers",
                                        self.task_config, "cross_num_hidden_layers")  # 用任务配置中的跨模态编码器层数替换跨模态编码器配置中的层数
            self.cross = CrossModel(cross_config)  # 跨模态编码器初始化
            # <=== End of Cross Encoder

            if self.train_sim_after_cross is False:
                # Decoder ===>
                decoder_config = update_attr("decoder_config", decoder_config, "num_decoder_layers",
                                           self.task_config, "decoder_num_hidden_layers")  # 用任务配置中的解码器层数替换解码器配置中的层数
                self.decoder = DecoderModel(decoder_config, bert_word_embeddings_weight, bert_position_embeddings_weight)  # 解码器初始化。使用BERT的嵌入权重和位置编码作为解码器的嵌入和位置编码
                # <=== End of Decoder

            if self.task_config.do_pretrain:  # 是否进行预训练
            ...
            self.decoder_loss_fct = CrossEntropyLoss(ignore_index=-1)  # -1是未被掩码词汇的id，忽略掉-1，只对掩码词汇进行预测

        self.normalize_video = NormalizeVideo(task_config)  # LN

        mILNCELoss = MILNCELoss(batch_size=task_config.batch_size // task_config.n_gpu, n_pair=task_config.n_pair, )  # MILNCE损失
        maxMarginRankingLoss = MaxMarginRankingLoss(margin=task_config.margin,
                                                    ...
                                                    hard_negative_rate=task_config.hard_negative_rate, )  # maxmarginrankingloss：L=max(0,-y*(x1-x2)+margin)。当y=1时，x1应排在x2的前面，当y=-1时，x1应排在x2后面
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,
        ...
        input_ids = input_ids.view(-1, input_ids.shape[-1])  # 压掉中间一维，即k的那一维
        token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1])  # 同上，token_type_ids即segment
        attention_mask = attention_mask.view(-1, attention_mask.shape[-1])  # 同上，attention_mask即MLM的padding mask
        video_mask = video_mask.view(-1, video_mask.shape[-1])  # 同上
        video = self.normalize_video(video)  # 特征归一化

        if input_caption_ids is not None:
            input_caption_ids = input_caption_ids.view(-1, input_caption_ids.shape[-1])  # 同上
            decoder_mask = decoder_mask.view(-1, decoder_mask.shape[-1])  # 同上

        sequence_output, visual_output = self.get_sequence_visual_output(input_ids, token_type_ids, attention_mask,
                                                                         video, video_mask, shaped=True)  # 文本编码、视频编码

        if self.training:  # 前面设置了model.train()，故self.training=True
            loss = 0.
            if self._stage_one:
            ...
            if self._stage_two:
                if self.task_config.do_pretrain:  # 预训练任务
                ...
                if (input_caption_ids is not None) and \
                    ...
                    elif self.task_config.task_type == "caption":  # 非预训练的caption任务
                        decoder_scores, res_tuples = self._get_decoder_score(sequence_output, visual_output,
                                                                             input_ids, attention_mask, video_mask,
                                                                             input_caption_ids, decoder_mask, shaped=True)  # 解码，得到生成文本各个词汇的得分
                    else:
                        raise NotImplementedError

                    output_caption_ids = output_caption_ids.view(-1, output_caption_ids.shape[-1])  # 压掉第二个维度
                    decoder_loss = self.decoder_loss_fct(decoder_scores.view(-1, self.bert_config.vocab_size), output_caption_ids.view(-1))
                    loss += decoder_loss

                if self.task_config.do_pretrain or self.task_config.task_type == "retrieval":  # 预训练/检索任务
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False):
        ...
        encoded_layers, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True)  # 文本编码，得到所有层的隐层输出
        sequence_output = encoded_layers[-1]  # 编码结果，最后一层输出

        visual_layers, _ = self.visual(video, video_mask, output_all_encoded_layers=True)  # 视频帧编码，得到所有隐层输出
        visual_output = visual_layers[-1]  # 编码结果，最后一层输出
    ...
    def _get_cross_output(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        text_type_ = torch.zeros_like(attention_mask)  # 文本数据类型编码
        video_type_ = torch.ones_like(video_mask)  # 图像数据类型编码
        concat_type = torch.cat((text_type_, video_type_), dim=1)

        cross_layers, pooled_output = self.cross(concat_features, concat_type, concat_mask, output_all_encoded_layers=True)  # 跨模态信息编码，得到跨模态编码器每一层的输出和文本CLS词元编码+线性变换后的输出
        cross_output = cross_layers[-1]  # 取编码器最后一层输出作为编码结果
    ...
    def _get_decoder_score(self, sequence_output, visual_output, input_ids, attention_mask, video_mask, input_caption_ids, decoder_mask, shaped=False):
        ...
        if shaped is False:  # 若第二个维度还未被压掉
            input_ids = input_ids.view(-1, input_ids.shape[-1])  # 压掉第二个维度，下同
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        res_tuples = ()  # 无用
        cross_output, pooled_output, concat_mask = self._get_cross_output(sequence_output, visual_output, attention_mask, video_mask)  # 跨模态编码输出，跨模态全局信息聚合器，跨模态padding mask
        decoder_scores = self.decoder(input_caption_ids, encoder_outs=cross_output, answer_mask=decoder_mask, encoder_mask=concat_mask)  # 解码，得到生成文本的各个词汇得分
    ...
    def decoder_caption(self, sequence_output, visual_output, input_ids, attention_mask, video_mask, input_caption_ids, decoder_mask,
        ...
        decoder_scores, _ = self._get_decoder_score(sequence_output, visual_output,
                                                    input_ids, attention_mask, video_mask,
                                                    input_caption_ids, decoder_mask, shaped=True)  # 得到各beam解码得分
