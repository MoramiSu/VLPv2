...
class BertOnlyMLMHead(nn.Module):
    def __init__(self, config, decoder_model_embedding_weights):
        ...
        self.predictions = BertLMPredictionHead(config, decoder_model_embedding_weights)  # 进行一些变换后线性映射到词表长度
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)  # 各头维度
        self.all_head_size = self.num_attention_heads * self.attention_head_size  # 隐层维度

        self.query = nn.Linear(config.hidden_size, self.all_head_size)  # q映射
        self.key = nn.Linear(config.hidden_size, self.all_head_size)  # k映射
        self.value = nn.Linear(config.hidden_size, self.all_head_size)  #v映射
...
class DecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.intermediate = BertIntermediate(config) # 升维
        self.output = BertOutput(config)  # 降维

    def forward(self, dec_input, enc_output, slf_attn_mask=None, dec_enc_attn_mask=None):
        slf_output, _ = self.slf_attn(dec_input, dec_input, dec_input, slf_attn_mask)  # 对输入ground truth进行掩码自注意力机制编码，每个词只能attend到其之前的词
        dec_output, dec_att_scores = self.enc_attn(slf_output, enc_output, enc_output, dec_enc_attn_mask)  # 使用输入ground truth编码作为q，跨模态编码器输出为kv进行自注意力机制（非mask，只考虑padding mask，因为输入ground truth编码已经mask过了）
...
class DecoderEmbeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        words_embeddings = self.word_embeddings(input_ids)  # 使用文本编码器的嵌入权重进行嵌入
        position_embeddings = self.position_embeddings(position_ids)  # 使用文本编码器的位置编码作为解码器的位置编码
...
class Decoder(nn.Module):
    ...
    def forward(self, hidden_states, encoder_outs, self_attn_mask, attention_mask, output_all_encoded_layers=False):
        dec_att_scores = None
        all_encoder_layers = []  # 隐层输出
        all_dec_att_probs = []  # 注意力权重
...
class DecoderModel(PreTrainedModel):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight):
        ...
        self.config = config
        self.max_target_length = config.max_target_embeddings  # 解码文本最大长度
        self.embeddings = DecoderEmbeddings(config, decoder_word_embeddings_weight, decoder_position_embeddings_weight)  # 使用BERT的嵌入权重和位置编码作为解码器的嵌入权重和位置编码
        self.decoder = Decoder(config)
        self.classifier = DecoderClassifier(config, decoder_word_embeddings_weight)  # 一些变换后线性映射到词表长度
    ...
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None):
        ...
        """
        embedding_output = self.embeddings(input_ids)  # 对decoder输入的ground truth进行编码

        extended_encoder_mask = encoder_mask.unsqueeze(1).unsqueeze(2)  # 编码器输出的padding mask   # b x 1 x 1 x ls
        extended_encoder_mask = extended_encoder_mask.to(dtype=self.dtype) # fp32 compatibility
        extended_encoder_mask = (1.0 - extended_encoder_mask) * -10000.0

        extended_answer_mask = answer_mask.unsqueeze(1).unsqueeze(2)  # 解码器输入的padding mask
        extended_answer_mask = extended_answer_mask.to(dtype=self.dtype)  # fp32 compatibility

        sz_b, len_s, _ = embedding_output.size()  # sz_b：bs，len_s：文本长度
        subsequent_mask = torch.triu(torch.ones((len_s, len_s), device=embedding_output.device, dtype=embedding_output.dtype), diagonal=1)  # 得到一个文本长度×文本长度的全1上三角矩阵，diagonal=1表示主对角线之上的第一条对角线，该线下的所有元素均为0
        self_attn_mask = subsequent_mask.unsqueeze(0).expand(sz_b, -1, -1).unsqueeze(1)  # 增加两个维度并复制上三角句子sz_b次  # b x 1 x ls x ls
        slf_attn_mask = ((1.0 - extended_answer_mask) + self_attn_mask).gt(0).to(dtype=self.dtype)  # 将self_attn_mask的padding部分全部置1
        self_attn_mask = slf_attn_mask * -10000.0
        # 这样得到的attn_mask，第一行第一个元素为0，其后均为-10000，第二行前两个元素均为0，其后均为-10000，以此类推，而padding部分则始终为-10000
        # 与注意力得分句子相加后，第一行只有第一位有注意力得分，其后都是小值，第二行只有前两位有注意力得分，其后均为小值，以此类推，同理，padding始终为小值
        # 经过softmax后，小值均为0，第一行只有第一个元素有注意力得分，第二行只有前两位有注意力得分，即第一个词只能attend到第一个词，第二个词则可以attend到前两个词，以此类推，实现掩码注意力机制
        ...
        sequence_output = decoded_layers[-1]  # 取最后一层输出作为解码结果
        cls_scores = self.classifier(sequence_output)  # 线性分类
