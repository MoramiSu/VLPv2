...
class Pipeline(_ScikitCompat):
    ...
    @abstractmethod
    def _forward(self, input_tensors: Dict[str, GenericTensor], img0, img1, **forward_parameters: Dict) -> ModelOutput:
        """
    ...
    def forward(self, model_inputs, img0, img1, **forward_params):
        with self.device_placement():
            if self.framework == "tf":
                model_inputs["training"] = False
                model_outputs = self._forward(model_inputs, img0, img1, **forward_params)
            elif self.framework == "pt":
                inference_context = self.get_inference_context()
                with inference_context():
                    model_inputs = self._ensure_tensor_on_device(model_inputs, device=self.device)
                    img0 = self._ensure_tensor_on_device(img0, device=self.device)
                    img1 = self._ensure_tensor_on_device(img1,device=self.device)
                    model_outputs = self._forward(model_inputs, img0, img1, **forward_params)
    ...
    def __call__(self, inputs, imgs, *args, num_workers=None, batch_size=None, **kwargs):
        if args:
        ...
        if is_list:
            ...
            return self.run_single(inputs, imgs, preprocess_params, forward_params, postprocess_params)

    def run_multi(self, inputs, preprocess_params, forward_params, postprocess_params):
        return [self.run_single(item, preprocess_params, forward_params, postprocess_params) for item in inputs]

    def run_single(self, inputs, imgs, preprocess_params, forward_params, postprocess_params):
        model_inputs = self.preprocess(inputs, **preprocess_params)
        model_outputs = self.forward(model_inputs, imgs[0], imgs[1], **forward_params)
