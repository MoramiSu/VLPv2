...
class TransModel2d(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.dense = InputDense2d(config)  # 将输入图像的一个patch线性映射成一个隐层维度的张量，即patch嵌入
        self.embeddings = TransEmbeddings(config)  # 位置编码
        self.encoder = TransEncoder(config)  # transformer encoder
