...
class MGCA(LightningModule):
    ...
    def __init__(self,
        ...
        # self.query = nn.Parameter(torch.randn([32, 768]))
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        scores0 = cl_img0_feat.mm(cl_text_feat.t())/self.hparams.softmax_temperature
        scores1 = cl_img1_feat.mm(cl_text_feat.t())/self.hparams.softmax_temperature
        ...
        sim_mat0 = torch.matmul(qcl_text_feat, torch.transpose(qcl_q_feat0, 1, 2)) / self.hparams.softmax_temperature
        qscores0 = torch.max(sim_mat0, dim=-1)[0]
        sim_mat1 = torch.matmul(qcl_text_feat, torch.transpose(qcl_q_feat1, 1, 2)) / self.hparams.softmax_temperature
        qscores1 = torch.max(sim_mat1, dim=-1)[0]
        qscores = (qscores0 + qscores1)/2
        qscores_t = qscores.transpose(0, 1)
        ...
        if batch['text1'] != None:
            output = self.decoder(input_ids=batch['text1'], attention_mask=batch['attn_mask1'],
                               encoder_hidden_states=q_feat, labels=batch['label1'])
            t_loss1 = output['loss']
        else:
            t_loss1 = torch.tensor([0]).type_as(t_loss0)

        if batch['text2'] != None:
            output = self.decoder(input_ids=batch['text2'], attention_mask=batch['attn_mask2'],
                               encoder_hidden_states=q_feat, labels=batch['label2'])
            t_loss2 = output['loss']
    ...
    def generate(self, input_ids, attn_mask, img0, img1, **kwargs):
        ...
        output = self.decoder.generate(input_ids=input_ids, attention_mask=attn_mask,
                              encoder_hidden_states=q_feat.contiguous(), **kwargs)
        return output
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--lambda_0", type=float, default=3.)
