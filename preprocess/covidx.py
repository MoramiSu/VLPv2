...
def preprocess_covidx():
    ...
    raw_train_df.columns = ['patient id', 'filename', 'class', 'data source']  # 给各列加title

    # Since we are doing image classification, patient id and data source is of no importance to us, so
    # we cn drop them
    raw_train_df = raw_train_df.drop(['patient id', 'data source'], axis=1)  # 丢弃无用列
    raw_train_df["labels"] = raw_train_df["class"].apply(set_label)  # 将class列进行数字化标注作为新的labels列
    # raw_train_df["labels"] = (raw_train_df["class"] ==
    #                           "positive").astype(np.float32)

    # split into train and test
    train_df, valid_df = train_test_split(  # 数据划分
