...
def dice_loss(pred, target):
    ...
    pred = torch.sigmoid(pred)  # 转化为概率

    p_flat = pred.view(-1)  # 拉成1维
    t_flat = target.view(-1)
    intersection = (p_flat * t_flat).sum()  # 取交
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        if not (target.size() == input.size()):  # 图片大小不同
            raise ValueError(
                "Target size ({}) must be the same as input size ({})".format(
                    target.size(), input.size()
                )
            )
        max_val = (-input).clamp(min=0)  # 将input的正数置0  # ?
        loss = (
            input
            - input * target
            + max_val
